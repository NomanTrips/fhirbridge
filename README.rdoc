...
==API server
-Clone the repository to a folder on your desktop (you could also download the zip instead):
 open a terminal or cmd prompt.
 cd to the folder you want to clone the repository into.
 run the command: "git clone https://github.com/NomanTrips/alpha-chase.git".
...
-Install the api server: In the project folder at the top node where the 'app' folder is run this command: "bundle install"
 This will install the project gems.
-Upload to Heroku
 Create an account on heroku or login.
 Inside the project folder top node enter the following commands:
 git init
 git add .
 git commit -m "init"
 heroku create
...
-Enter the following command "heroku addons:add heroku-postgresql --app your-app-name --version=9.4" to add a postgres db to the app.
...
-Run "curl https://raw.githubusercontent.com/fhirbase/fhirbase-build/master/fhirbase.sql \ | pg:psql --app your-app-name YOUR_DB_NAME"
You will need to download curl for this step. If your on windows you can do this with Cygwin.
-Run "heroku pg:psql --app your-app-name YOUR_DB_NAME --command 'SELECT fhir.generate_tables()'" this will setup the database and add the necessary tables.
